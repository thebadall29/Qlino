    ...
    try {
      if (isLogin) {
        ...
        if (data.user.role !== 'patient') {
          throw new Error('This login is for patients only. Please use the appropriate login page.');
        }
        ...
        const data = await response.json();
        console.log('Registration response:', data); // Add this line for debugging registratio
        
        if (!response.ok) {
          throw new Error(data.message || 'Registration failed');
        }
        
        setSuccess('Registration successful! Please check your email for verification. You can now log in.');
  ...
  return (
    <div className="patient-auth-container">
      <div className="auth-card">
        ...
        <h2 className="auth-title">{isLogin ? 'Patient Login' : 'Patient Signup'}</h2>
